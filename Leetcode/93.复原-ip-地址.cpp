...
class Solution {
  ...
  public:
    vector<string> ans;
    void dfs(string s, int start, int cnt, vector<string> &tmp) {
        if (cnt == 3) {
            if (s.size() - start > 3 || s.substr(start) > "255") {
                return;
            }
            string t;
            for (int i = 0; i < 3; i++) {
        ...
        int sum = 0;
        for (int i = start, j = 0; i < s.size() && j < 3; i++, j++) {
            sum = sum * 10 + (s[i] - '0');
            if (sum <= 255) {
                tmp.push_back(s.substr(start, j + 1));
                dfs(s, i + 1, cnt + 1, tmp);
                tmp.pop_back();
            }
        }
